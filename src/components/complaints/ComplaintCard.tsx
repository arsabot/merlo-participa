...
export const ComplaintCard: React.FC<ComplaintCardProps> = ({ complaint }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E8E4EF] p-5 sm:p-6 shadow-lla-soft hover:shadow-lla-card transition-all duration-300 hover:border-purple-300 flex flex-col justify-between group">
      <div>
        ...
        <Link href={`/reclamos/${complaint.id}`} className="block">
          <h3 className="text-lg font-bold text-[#17151D] group-hover:text-[#391759] transition-colors leading-snug line-clamp-2">
            {complaint.title}
          </h3>
        </Link>

        {/* Description snippet */}
        <p className="mt-2 text-sm text-[#6B6875] line-clamp-3 leading-relaxed">
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-[#6B6875]">
          ...
          {complaint.updates && complaint.updates.length > 1 && (
            <div className="flex items-center gap-1 text-[#391759] font-medium">
      ...
      <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
        ...
        <Link
          ...
          className="inline-flex items-center gap-1 text-xs font-bold text-[#391759] hover:text-[#240c3a] group-hover:translate-x-0.5 transition-transform"
        >
